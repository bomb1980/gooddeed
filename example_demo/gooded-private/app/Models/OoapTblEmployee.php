...
class OoapTblEmployee extends Authenticatable
{
    ...
    public static function updateEmployees($user)
    {
        $employee_api = Http::withHeaders([
            ...
            ->post(config('app.umts_api') . '/eis/employee_deteil_citizen_id', [
                'em_citizen_id' => $user->emp_citizen_id,
            ])
            ->json();
        ...
        if (isset($employee_api[0]['division_id'])) {
            ...
            foreach (
                OoapMasDivision::getDatas(
                    $employee_api[0]['division_id']
                )->get()
                as $kp => $vp
            ) {
    ...
    public static function readNoti()
    {
        return self::where(
            'emp_citizen_id',
            '=',
            auth()->user()->emp_citizen_id
        )->update(['new_noti' => 0]);
    }

    public function users()
    {
        return $this->belongsToMany(OoapTblEmployee::class)->withTimestamps();
    }
